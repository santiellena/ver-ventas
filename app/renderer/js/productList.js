...
async function checkProductExistense() {
  ...
  const product = getItemSession(idProduct, key);
  if (product != null && product != undefined) {
    let actualProductQuantity = document.getElementById(
      `quantArticle${idProduct}`
    ).value;
    actualProductQuantity++;
    updateItemSession(idProduct, actualProductQuantity, key);
    ...
    if (checkPriceList() == "public") {
      let newSubTotal = 0;
      if (product.onSale == 1) {
        const discount = await ipcRenderer.invoke(
          "get-product-discount",
          idProduct
        );
        newSubTotal =
          actualProductQuantity * product.unitPrice -
          discount * actualProductQuantity;
...
async function addProduct() {
  ...
  if (product.id == null || product.id == undefined) {
    ...
    setItemSession(id, quantity, key);
...
async function searchProduct() {
  ...
  const productData = await ipcRenderer.invoke(
    "search-product-byid",
    idProduct
  );

  showProduct.value = productData.description
...
async function updateAmountByInput(id) {
  ...
  updateItemSession(id, input.value, key);
  subTotal.innerText = `$ ${newSubTotal}`;
  updateSubTotal();
}
...
ipcRenderer.on("clear-product-list", () => {
  clearProductList(key);
  updateSubTotal();
});

function deleteProductList(id) {
  removeItemSession(id, key);
...
ipcRenderer.on("add-product-tosell-list", async () => {
  ...
  const sessionProduct = getItemSession(idProduct, key);
  if (sessionProduct != null && sessionProduct != undefined) {
    ...
    if (checkPriceList() == "public") {
      ...
      const newSubTotal =
        product.unitPrice * actualProductQuantity -
        discount * actualProductQuantity;
      quantityInput.value = actualProductQuantity;
      subTotal.innerText = `$ ${newSubTotal}`;
      discountElement.innerText = `$ ${discount * actualProductQuantity}`;
    ...
    const quantity = 1;
    let discount = 0;
    setItemSession(idProduct, quantity, key);
    if (product.onSale == 1) {
      if (checkPriceList() == "public") {
        discount = await ipcRenderer.invoke("get-product-discount", product.id);
      }
...
ipcRenderer.on("get-sells-details", () => {
  const sessionStorage = getAllItemSession(key);
